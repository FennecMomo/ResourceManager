...
public sealed class ReminderService(NodeStore store, PeerClient client)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!Valid(request.MessageId, 64) || !Valid(request.SenderDeviceId, 100) || !Valid(request.ResourceId, 100) ||
            !Valid(request.ResourceName, MaxNameLength) || request.Note.Length > MaxNoteLength || request.SentUtc == default)
        ...
        lock (gate)
        {
            ...
            lastSameResource[dedupeKey] = now;
            lastAccepted[sender.DeviceId] = now;
            if (!acceptedHistory.TryGetValue(sender.DeviceId, out var history))
            {
            ...
            history.Enqueue(now);
            while (history.Count > 0 && now - history.Peek() >= HourlyWindow) history.Dequeue();
        }

        ReminderReceived?.Invoke(new ReminderDelivery(sender, resource, request.SentUtc, now));
