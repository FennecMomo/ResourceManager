...
> 状态：历史设计稿 · 原目标版本 0.3.0 已失效 · 2026-09-21
>
> 2026-09-22：新的多服务器、在线名单、“发布/上传”双模式需求已记录在 [开发待办与版本路线](roadmap.md)。本稿的“所有流量经大厅中继”与新需求中的发布资源传输路线尚未统一，不能直接按本稿进入实现。
...
- 本稿原计划使用 **0.3.0**，该版本号现已用于局域网发现与下载任务移除，原计划作废。
- 后续服务器功能按 [开发待办与版本路线](roadmap.md) 重新拆分版本；服务端项目名称、构建产物和部署方式在进入服务器阶段时确定。
- 本稿仍可作为中继协议研究资料，但不能直接作为当前版本计划或验收依据。
