...
当前客户端开发版本为 `0.3.6`，服务端版本为 `0.0.1`，两者独立演进。客户端发布使用 `v系统.模块.修改` 标签，服务端发布使用 `server-v系统.模块.修改` 标签。图标的矢量原稿位于 `assets/icon.svg`，使用 `python -m pip install pillow cairosvg` 和 `python tools/render_icon.py` 可重新生成 Windows 图标。变更记录见 [CHANGELOG.md](CHANGELOG.md)。项目采用 [MIT 许可证](LICENSE)。本机 SSH 密钥、签名文件和打包产物均不提交到仓库。
