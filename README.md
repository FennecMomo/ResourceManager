...
0.3.3 起可以为设备保存本机备注（只有自己能看到，不改变对方昵称），为已发布的资源填写备注（随资源目录显示给已连接设备），并把某个可用资源提醒给指定的在线设备。提醒只在对方软件内弹出，是否下载以及保存到哪里由对方决定；接收方可以在弹窗上对单台设备临时静音 1 小时，关闭提醒后不保留记录。0.3.2 及更早版本仍可连接、浏览和下载，但不会显示备注，也不参与提醒。
