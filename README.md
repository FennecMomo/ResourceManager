...
3. 在“设备”中点击“查找局域网设备”，软件会自动发现并连接同一局域网内正在运行的设备；也可以输入对方的 IPv4 地址和端口手动连接。双方会自动记录彼此的昵称和地址，然后可浏览、收藏或下载对方发布的资源。
4. 如果设备隔着一台路由器，在“设备”的“路由器入口”中填写该路由器的 WAN IPv4。软件会先检查已经保存的端口，再扫描固定范围 `48001–48099`，通过一台已映射的设备发现路由器内的其他 ResourceManager 设备。路由器内部设备可在“设置”的“当前路由器”卡片复制外部入口 IP。
